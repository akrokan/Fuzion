...
<div class="bg-white items-center max-w-xl p-2 shadow-lg space-y-4 ">
    ...
    <form method="POST" action="{{ url('/dashboard/post/edit') }}" enctype="multipart/form-data">
        @csrf
        <span class="block mb-2">
            <label for="title" id="title">Title</label>
            <input type="text" name="title" class="border px-2 py-1 shadow-inner w-full" value="{{ $post->title }}">
        </span>
        <span class="block mb-2">
            <label for="slug" id="slug">Slug</label>
            <input type="text" name="slug" class="border px-2 py-1 shadow-inner w-full" value="{{ $post->slug }}">
        </span>
        <span class="block mb-2">
        ...
        <span class="block mb-2">
            <textarea class="w-full">{{ $post->content }}</textarea>
        </span>
